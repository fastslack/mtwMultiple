...
class JInstallationHelper
{
	...
	function setDirPerms($dir, &$srv)
	{
		 ...
		 */
		if (!JPath::canChmod(JPath::clean(JPATH_SITE.'/'.$dir)))
		{
			$ftpFlag = true;
		}
		...
		else
		{
			...
			$path = JPath::clean(JPATH_SITE.'/'.$dir);
	...
	function uploadSql( &$args, $migration = false, $preconverted = false )
	{
		...
		JInstallationHelper::_chmod(JPATH_SITE.'/tmp', 0777);
		jimport('joomla.filesystem.file');
		$uploaded = JFile::upload($sqlFile['tmp_name'], JPATH_SITE.'/tmp/'.sqlFile['name']);
		if(!$uploaded) {
		...
		{
			$archive = JPATH_SITE.'/tmp/'.sqlFile['name'];
		}
		else
		{
			$script = JPATH_SITE.'/tmp/'.sqlFile['name'];
		}
	...
	function preMigrate( $scriptName, &$args, $db )
	{
		...
		$newFile = dirname( $scriptName ).'/converted.sql';
	...
	function postMigrate( $db, & $errors, & $args ) {
		...
		foreach( $lookup as $module )
		{
			...
			if ( $row = $db->loadObject() ) {
				...
				if(JFolder::exists(JPATH_SITE.'/modules/'.row->module)) {
					$nextId++;
					$oldid = $row->id;
					$row->id = $nextId;
					$row->published = 0;
